...
Get your Tiingo token from https://api.tiingo.com/account/api/token
Create a new repo on GitHub (e.g., `pf-detector`) and push this folder to it.
...
```
   ...
   ```
In Vercel, click "Add New Project," import the repo you just created, and
click Deploy. The first build will succeed but the app will return an error
until step 4.
In the Vercel project settings, go to Settings → Environment Variables and
add two variables:
...
Trigger a redeploy from the Vercel dashboard (Deployments tab → ⋯ →
...
Open the URL.
Click "Pull from Tiingo" → enter the shared access password → click Start
pull. The app fetches 25 tickers in about 15 seconds, dots turn green.
Click "Detect pivots." Open the Signals tab to see what's at or past its
